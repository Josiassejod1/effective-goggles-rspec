...
module ExpenseTracker
  RSpec.describe Ledger, :aggregate_failures, :db do
    ...
    describe '#record' do
      ...
      context 'when expense lacks a date' do
        it 'rejects the expense as invalid' do
          expense.delete('date')
      ...
      context 'when all values missing' do
        it 'rejects the expense as invalid' do
          expense = {}
          ...
          expect(result).not_to be_success
          expect(result.expense_id).to eq(nil)
          expect(result.error_message).to include('payee, date, amount is required')
          expect(DB[:expenses].count).to eq(0)
        end
      end
    end
    ...
    describe '#expenses_on' do
      it 'returns all expenses for provided date' do
      ...
      it 'returns a blank array whem there are no matching expenses' do
